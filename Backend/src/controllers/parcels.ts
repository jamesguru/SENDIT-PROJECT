...
export const softDelete: RequestHandler<{ id: string }> = async (
  req: Request,
  res: Response
) => {
...
export const updateParcelStatus: RequestHandler<{ id: string }> = async (
  req: Request,
  res: Response
) => {
  ...
  try {
    ...
    await axios.post("http://localhost:8000/api/notifications", {
      trackId,
      email: receiverEmail,
      message: `The parcel ${trackId} has been delivered`,
    });
    await axios.post("http://localhost:8000/api/notifications", {
      trackId,
      email: senderEmail,
      message: `Your order ${trackId} has been delivered`,
    });
